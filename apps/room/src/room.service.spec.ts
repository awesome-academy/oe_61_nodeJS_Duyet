...
describe('RoomService', () => {
  ...
  // --- Test Suite for updateRoomImage ---
  describe('updateRoomImage', () => {
    it('should update image successfully', async () => {
      roomRepository.update.mockResolvedValue({ affected: 1 } as UpdateResult);

      await service.updateRoomImage(1, 'url');

      expect(roomRepository.update).toHaveBeenCalledWith(1, { image: 'url' });
    });

    it('should catch and log error if update fails', async () => {
      roomRepository.update.mockRejectedValue(new Error('fail'));
      await service.updateRoomImage(1, 'url');
